                ...
                children: [
                  ...
                  if (needsConfirmation)
                    Row(
                      children: [
                        IconButton(
                          icon: const Icon(Icons.check_circle, color: AppColors.sageGreen),
                          onPressed: () => controller.confirmDone(item, true),
                        ),
                        IconButton(
                          icon: const Icon(Icons.cancel, color: AppColors.emberCoral),
...
class _StatusPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final label = item.isConfirmed == null ? 'Upcoming' : (item.isConfirmed! ? 'Done' : 'Missed');
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.event_available_rounded, size: 56, color: Colors.grey),
            const SizedBox(height: 12),
            const Text('No occasional reminders yet', style: TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
            const Text(
  ...
  Get.dialog(
    AlertDialog(
        ...
        child: Column(
          ...
          children: [
            ...
            Obx(() => ListTile(
                  ...
                  title: Text(date.value == null ? 'Pick Date' : date.value!.toLocal().toString().split(' ')[0]),
                  onTap: () async {
            ...
            Obx(() => ListTile(
                  ...
                  title: Text(time.value == null ? 'Pick Time (optional)' : time.value!.format(context)),
                  onTap: () async {
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            if (titleCtrl.text.trim().isEmpty || date.value == null) {
              Get.snackbar('Missing info', 'Title and date are required');
              return;
            }
